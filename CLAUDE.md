...
- **Fase**: Módulo 1 + Módulo 2 completos · pronto para deploy e beta com contadores reais
- **Empresa**: ativa, CNAE a alterar, endereço fiscal a contratar
- **Produto**: app completo — auth, onboarding, painel, clientes, calendário, configurações, busca Spotlight, sidebar com identidade do escritório. Infraestrutura de vencimentos completa: geração ao criar/editar cliente, Vercel Cron mensal, pg_cron marcar-atrasados e alertas-diarios ativos, feriados 2026/2027 populados, alertas por e-mail via Resend+webhook implementados. **Módulo 2 (Portal do Cliente) implementado**: campo de valor nas obrigações, fluxo de convite por e-mail, portal para empresários em `/portal` com guard de auth, lista de obrigações e valores. **Página de Detalhes** (`/clientes/[id]/detalhes`): status cards glass, selection mode, calendário grade/lista. **Contato + Endereço**: campos opcionais no cadastro de clientes; lookup de CEP via BrasilAPI.
